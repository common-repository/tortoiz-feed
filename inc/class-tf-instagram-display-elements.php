<?php
/**
 * Class tf_Instagram_Display_Elements
 *
...
class tf_Instagram_Display_Elements
{
	...
	public static function get_item_classes( $settings ) {
		if ( !$settings['disable_js_image_loading'] ) {
			return ' tfi_new tfi_transition';
		} else {
			return ' tfi_new tfi_no_js tfi_no_resraise tfi_js_load_disabled';
		}
	}
	...
	public static function get_optimum_media_url( $post, $settings, $resized_images = array() ) {
		...
		if ( !$settings['disable_js_image_loading'] ) {
			return trailingslashit( tfi_PLUGIN_URL ) . 'img/placeholder.png';
			...
			if ( $settings['imageres'] !== 'thumb' && ! empty( $resized_images ) ) {
				...
				if ( isset( $resized_images[ $post_id ] )
					...
					if ( isset( $suffix ) ) {
						$media_url = tfi_get_resized_uploads_url() . $resized_images[ $post_id ]['id'] . $suffix . '.jpg';
						return $media_url;
					}
				}
			}
		...
		if ( $account_type === 'personal' ) {
			...
			if ( $optimum_res === 'full' && isset( $resized_images[ $post_id ]['id'] )
			     ...
			     && $resized_images[ $post_id ]['id'] !== 'video'
			     && $resized_images[ $post_id ]['id'] !== 'error' ) {
				$media_url = tfi_get_resized_uploads_url() . $resized_images[ $post_id ]['id'] . 'full.jpg';
			} else {
				$permalink = tf_Instagram_Parse::get_permalink( $post );
	 ...
	 */
	public static function get_tfi_photo_style_element( $post, $settings, $resized_images = array() ) {
		if ( !$settings['disable_js_image_loading'] ) {
			return '';
		} else {
			$full_res_image = tf_Instagram_Display_Elements::get_optimum_media_url( $post, $settings, $resized_images );
			/*
	...
	public static function get_feed_style( $settings ) {
		...
		if ( ! empty( $settings['imagepadding'] )
			...
			if ( ! empty( $settings['background'] ) ) {
				$styles .= 'background-color: rgb(' . esc_attr( tfi_hextorgb( $settings['background'] ) ). ');';
			}
	...
	/**
	 * Creates a style attribute for the tfi_images div
	 *
	 * @param $settings
	 *
	 * @return string
	 */
	public static function get_tfi_images_style( $settings ) {
		if ( ! empty ( $settings['imagepadding'] ) ) {
	...
	public static function get_header_text_color_styles( $settings ) {
		if ( ! empty( $settings['headercolor'] ) ) {
			return 'style="color: rgb(' . esc_attr( tfi_hextorgb( $settings['headercolor'] ) ). ');"';
		}
		return '';
	}
	...
	public static function get_header_size_class( $settings ) {
		$header_size_class = in_array( strtolower( $settings['headersize'] ), array( 'medium', 'large' ) ) ? ' tfi_'.strtolower( $settings['headersize'] ) : '';
	...
	public static function get_follow_styles( $settings ) {
		...
		if ( ! empty( $settings['followcolor'] ) || ! empty( $settings['followtextcolor'] ) ) {
			...
			if ( ! empty( $settings['followcolor'] ) ) {
				$styles .= 'background: rgb(' . esc_attr( tfi_hextorgb( $settings['followcolor'] ) ) . ');';
			}
			if ( ! empty( $settings['followtextcolor'] ) ) {
				$styles .= 'color: rgb(' . esc_attr( tfi_hextorgb( $settings['followtextcolor'] ) ). ');';
			}
			$styles .= '"';
		}
		return $styles;
	...
	public static function get_load_button_styles( $settings ) {
		...
		if ( ! empty( $settings['buttoncolor'] ) || ! empty( $settings['buttontextcolor'] ) ) {
			...
			if ( ! empty( $settings['buttoncolor'] ) ) {
				$styles .= 'background: rgb(' . esc_attr( tfi_hextorgb( $settings['buttoncolor'] ) ) . ');';
			}
			if ( ! empty( $settings['buttontextcolor'] ) ) {
				$styles .= 'color: rgb(' . esc_attr( tfi_hextorgb( $settings['buttontextcolor'] ) ). ');';
			}
			$styles .= '"';
		}
		return $styles;
	...
	protected static function get_basic_icons( $type, $icon_type ) {
		if ( $type === 'carousel' ) {
			if ( $icon_type === 'svg' ) {
				return '<svg class="svg-inline--fa fa-clone fa-w-16 tfi_lightbox_carousel_icon" aria-hidden="true" aria-label="Clone" data-fa-proƒcessed="" data-prefix="far" data-icon="clone" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
	                <path fill="currentColor" d="M464 0H144c-26.51 0-48 21.49-48 48v48H48c-26.51 0-48 21.49-48 48v320c0 26.51 21.49 48 48 48h320c26.51 0 48-21.49 48-48v-48h48c26.51 0 48-21.49 48-48V48c0-26.51-21.49-48-48-48zM362 464H54a6 6 0 0 1-6-6V150a6 6 0 0 1 6-6h42v224c0 26.51 21.49 48 48 48h224v42a6 6 0 0 1-6 6zm96-96H150a6 6 0 0 1-6-6V54a6 6 0 0 1 6-6h308a6 6 0 0 1 6 6v308a6 6 0 0 1-6 6z"></path>
	            </svg>';
			} else {
				return '<i class="fa fa-clone tfi_carousel_icon" aria-hidden="true"></i>';
			}

		} elseif ( $type === 'video' ) {
			if ( $icon_type === 'svg' ) {
				return '<svg style="color: rgba(255,255,255,1)" class="svg-inline--fa fa-play fa-w-14 tfi_playbtn" aria-label="Play" aria-hidden="true" data-fa-processed="" data-prefix="fa" data-icon="play" role="presentation" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path fill="currentColor" d="M424.4 214.7L72.4 6.6C43.8-10.3 0 6.1 0 47.9V464c0 37.5 40.7 60.1 72.4 41.3l352-208c31.4-18.5 31.5-64.1 0-82.6z"></path></svg>';
			} else {
				return '<i class="fa fa-play tfi_playbtn" aria-hidden="true"></i>';
			}
			...
			if ( $icon_type === 'svg' ) {
				return '<svg class="tfi_new_logo fa-instagram fa-w-14" aria-hidden="true" data-fa-processed="" aria-label="Instagram" data-prefix="fab" data-icon="instagram" role="img" viewBox="0 0 448 512">
	                <path fill="currentColor" d="M224.1 141c-63.6 0-114.9 51.3-114.9 114.9s51.3 114.9 114.9 114.9S339 319.5 339 255.9 287.7 141 224.1 141zm0 189.6c-41.1 0-74.7-33.5-74.7-74.7s33.5-74.7 74.7-74.7 74.7 33.5 74.7 74.7-33.6 74.7-74.7 74.7zm146.4-194.3c0 14.9-12 26.8-26.8 26.8-14.9 0-26.8-12-26.8-26.8s12-26.8 26.8-26.8 26.8 12 26.8 26.8zm76.1 27.2c-1.7-35.9-9.9-67.7-36.2-93.9-26.2-26.2-58-34.4-93.9-36.2-37-2.1-147.9-2.1-184.9 0-35.8 1.7-67.6 9.9-93.9 36.1s-34.4 58-36.2 93.9c-2.1 37-2.1 147.9 0 184.9 1.7 35.9 9.9 67.7 36.2 93.9s58 34.4 93.9 36.2c37 2.1 147.9 2.1 184.9 0 35.9-1.7 67.7-9.9 93.9-36.2 26.2-26.2 34.4-58 36.2-93.9 2.1-37 2.1-147.8 0-184.8zM398.8 388c-7.8 19.6-22.9 34.7-42.6 42.6-29.5 11.7-99.5 9-132.1 9s-102.7 2.6-132.1-9c-19.6-7.8-34.7-22.9-42.6-42.6-11.7-29.5-9-99.5-9-132.1s-2.6-102.7 9-132.1c7.8-19.6 22.9-34.7 42.6-42.6 29.5-11.7 99.5-9 132.1-9s102.7-2.6 132.1 9c19.6 7.8 34.7 22.9 42.6 42.6 11.7 29.5 9 99.5 9 132.1s2.7 102.7-9 132.1z"></path>
	            </svg>';
			} else {
				return '<i class="tfi_new_logo"></i>';
			}
		} else {
			return '';
		}
