...
class tf_Instagram_Post
{
	...
	public function save_in_db( $transient_name = false, $timestamp_override = NULL ) {
		...
		if ( $error !== false ) {
			...
			$this->insert_tfi_instagram_feeds_posts( $transient_name );
		} else {
			// log error
		}
	...
	public function resize_and_save_image( $image_sizes, $upload_dir, $upload_url ) {
		if ( isset( $this->instagram_api_data['id'] ) ) {
			$image_source_set    = tf_Instagram_Parse::get_media_src_set( $this->instagram_api_data );
			$account_type        = tf_Instagram_Parse::get_account_type( $this->instagram_api_data );
			...
			if ( $account_type === 'business' ) {
				$new_file_name       = explode( '?', tf_Instagram_Parse::get_media_url( $this->instagram_api_data, 'lightbox' ) );
			...
			foreach ( $image_sizes_to_make as $res_setting => $image_size ) {
				if ( $account_type === 'business' ) {
					$file_name = tf_Instagram_Parse::get_media_url( $this->instagram_api_data, 'lightbox' );
				} else {
					$file_name = isset( $image_source_set[ $image_size ] ) ? $image_source_set[ $image_size ] : tf_Instagram_Parse::get_media_url( $this->instagram_api_data, 'lightbox' );
				}
				if ( ! empty( $file_name ) ) {
					...
					if ( ! is_wp_error( $image_editor ) ) {
						...
						if ( ! $saved_image ) {
							...
							$tf_instagram_posts_manager->add_error( 'image_editor_save', array(
								__( 'Error saving edited image.', 'tortoiz-instagram-feed' ),
								$full_file_name
							) );
						} else {
						...
						$message = __( 'Error editing image.', 'tortoiz-instagram-feed' );
						...
						$tf_instagram_posts_manager->add_error( 'image_editor', array( $file_name, $message ) );
					}
			...
			if ( $one_successful_image_resize ) {
				...
				$this->update_tfi_instagram_posts( array(
	...
	public function update_db_data( $update_last_requested = true, $transient_name = false, $image_sizes, $upload_dir, $upload_url, $timestamp_for_update = false ) {
		...
		if ( $this->media_id === 'pending' ) {
			...
			$this->update_tfi_instagram_posts( $to_update );
		}
	 ...
	 * @since 2.0/4.0
	 */
	public function update_tfi_instagram_posts( $to_update ) {
		global $wpdb;
		$table_name = $wpdb->prefix . tfi_INSTAGRAM_POSTS_TYPE;
		...
		foreach ( $to_update as $column => $value ) {
			...
			if ( $query === false ) {
				...
				$tf_instagram_posts_manager->add_error( 'database_update_post', array( __( 'Error updating post.', 'tortoiz-instagram-feed' ), $error . '<br><code>' . $query . '</code>' ) );
			}
		}
	...
	public function exists_in_feeds_posts_table( $transient_name ) {
		global $wpdb;
		$table_name = $wpdb->prefix . tfi_INSTAGRAM_FEEDS_POSTS;
	 ...
	 * @since 2.0/4.0
	 */
	public function insert_tfi_instagram_feeds_posts( $transient_name ) {
		global $wpdb;
		$table_name = $wpdb->prefix . tfi_INSTAGRAM_FEEDS_POSTS;
		...
		if ( ! empty( $this->db_id ) ) {
      	...
      	(id,instagram_id,feed_id) VALUES ($entry_string);" );
			...
			$tf_instagram_posts_manager->add_error( 'database_insert_post', array( __( 'Error inserting post.', 'tortoiz-instagram-feed' ), __( 'No database ID.', 'tortoiz-instagram-feed' ) ) );
			return false;
		}
		...
		if ( $error !== false ) {
			...
			$tf_instagram_posts_manager->add_error( 'database_insert_post', array( __( 'Error inserting post.', 'tortoiz-instagram-feed' ), $error . '<br><code>' . $query . '</code>' ) );
		}
	}
