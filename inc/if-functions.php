...
function display_instagram( $atts = array() ) {
	...
	if ( empty( $database_settings['connected_accounts'] ) && empty( $atts['accesstoken'] ) ) {
        ...
        <div id="sbi_mod_error" <?php echo $style; ?>>
            <span><?php _e('This error message is only visible to WordPress admins', 'tortoiz-feed' ); ?></span><br />
            <p><b><?php _e( 'Error: No connected account.', 'tortoiz-feed' ); ?></b>
            <p><?php _e( 'Please go to the Instagram Feed settings page to connect an account.', 'tortoiz-feed' ); ?></p>
        </div>
		<?php
...
function sbi_process_submitted_resize_ids() {
	...
	if ( empty( $database_settings['connected_accounts'] ) && empty( $atts['accesstoken'] ) ) {
		return '<div class="sb_instagram_error"><p>' . __( 'Please connect an account on the Instagram Feed plugin Settings page.', 'tortoiz-feed' ) . '</p></div>';
	}
