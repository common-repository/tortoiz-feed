 ...
 *
 * @wordpress-plugin
 * Plugin Name:       Tortoiz Feed
 * Description:       Tortoiz Feed plugin can show instagram feed with many custom designs.
 * Version:           1.2.3
 * Author:            Tortoiz Themes
 * Author URI:        https://tortoizthemes.com
 * Text Domain:       tortoiz-feed
...
if ( function_exists( 'sb_instagram_feed_init' ) ) {
	wp_die( "We are building Tortoiz Feed Premium version,if you have any suggestions to improve,please email us at tortoizthemes@gmail.com <a href='".get_admin_url(null, 'plugins.php')."'>Plugins page</a>." );
} else {
	/**
	...
	function sb_instagram_feed_init() {
		...
		require_once trailingslashit( SBI_PLUGIN_DIR ) . 'inc/class-tf-instagram-api-connect.php';
		require_once trailingslashit( SBI_PLUGIN_DIR ) . 'inc/class-tf-instagram-cron-updater.php';
		require_once trailingslashit( SBI_PLUGIN_DIR ) . 'inc/class-tf-instagram-display-elements.php';
		require_once trailingslashit( SBI_PLUGIN_DIR ) . 'inc/class-tf-instagram-feed.php';
		require_once trailingslashit( SBI_PLUGIN_DIR ) . 'inc/class-tf-instagram-parse.php';
		require_once trailingslashit( SBI_PLUGIN_DIR ) . 'inc/class-tf-instagram-post.php';
		require_once trailingslashit( SBI_PLUGIN_DIR ) . 'inc/class-tf-instagram-post-set.php';
		require_once trailingslashit( SBI_PLUGIN_DIR ) . 'inc/class-tf-instagram-posts-manager.php';
		require_once trailingslashit( SBI_PLUGIN_DIR ) . 'inc/class-tf-instagram-settings.php';
		...
		$sb_instagram_posts_manager = new tf_Instagram_Posts_Manager();
	}
				...
				if ( ! file_exists( $upload_dir ) ) {
					...
					if ( $created ) {
						...
						$sb_instagram_posts_manager->add_error( 'upload_dir', array(
							__( 'There was an error creating the folder for storing resized images.', 'tortoiz-feed' ),
							$upload_dir
						) );
					}
				} else {
		...
		if ( version_compare( $wp_version, '3.5', '<' ) ) {
			...
			if ( $wpdb->get_var( "show tables like '$table_name'" ) != $table_name ) {
				$sb_instagram_posts_manager->add_error( 'database_create_posts', array(
					__( 'There was an error when trying to create the database tables used for resizing images.', 'tortoiz-feed' ),
